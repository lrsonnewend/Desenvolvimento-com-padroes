...
public class RelatorioDAO {
	...
	public void servicoMaisProcurado(int opcao) {
		...
		try {
			...
			while (rs.next()) {
				System.out
						.println("Servico: (" + rs.getInt("id_servico") + "): " + rs.getString("nome_servico") + "\n");
			}
	...
	public void avgIdade(int op) {
		String sql = null;
		String where = "";
		Statement stmt = conDB.newStatement();
		ResultSet rs = null;

		if (op == 2)
			where = "WHERE c.genero_cliente = 'M'";
		if (op == 3)
			where = "WHERE c.genero_cliente = 'F'";

		sql = "SELECT avg(((julianday('now') - julianday(c.dataNasc_cliente))/(365))) as avgAge from clientes c " + where;
		
		try{
			rs = stmt.executeQuery(sql);
			
			while(rs.next()) {
				System.out.println(rs.getInt("avgAge"));
			}
			
		}catch(SQLException e) {
			System.err.println("erro ao calcular idade media: "+e);
		}
	...
	public String lidaServMaisProcurado(int op) {
		String sql = null;
		if (op == 1) {
			sql = "select cs.id_servico," + " s.nome_servico," + " count(cs.id_servico) as qtd_servico "
					+ "from cliente_servico cs LEFT JOIN servicos s on s.id_servico = cs.id_servico "
					+ "group by cs.id_servico " + "order by count(cs.id_servico) DESC LIMIT 1;"; // string para ser
																									// executada no
																									// banco de dados
		}
